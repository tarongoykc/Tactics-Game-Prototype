...
func _init(grid: Grid, walkable_cells: Dictionary) -> void:
	_grid = grid
	var cell_mappings := {}
	var cell_weights := {}
	for cell in walkable_cells:
		cell_mappings[cell] = _grid.as_index(cell)
		cell_weights[cell] = walkable_cells[cell]
	_add_and_connect_points(cell_mappings, cell_weights)
...
func _add_and_connect_points(cell_mappings: Dictionary, cell_weights: Dictionary) -> void:
	for point in cell_mappings:
		_astar.add_point(cell_mappings[point], point, cell_weights[point])
